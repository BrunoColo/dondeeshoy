          ...
          <div className="flex sm:hidden items-center gap-2 w-full">
            <div className="relative flex-1 min-w-0">
              ...
              <input
                ...
                className="w-full rounded-full bg-white/[0.08] border border-white/[0.14] pl-10 pr-4 py-2.5 text-[14px] text-foreground placeholder:text-text-muted focus:outline-none focus:border-neon-violet/45 focus:shadow-[0_0_14px_rgba(168,85,247,0.2)] transition-all duration-200"
              />
            </div>
            {searchValue.trim().length > 0 && (
              <button
                onClick={() => submitSearch(searchValue)}
                className="shrink-0 inline-flex h-9 items-center justify-center rounded-full bg-neon-violet/20 border border-neon-violet/30 px-3.5 text-[12px] font-semibold uppercase tracking-wide text-neon-violet hover:bg-neon-violet/30 transition-colors"
              >
                Ir
              </button>
            )}
